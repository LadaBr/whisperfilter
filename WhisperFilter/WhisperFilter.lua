...
local function onevent()
    ...
    if arg1 ~= nil then
    	...
    	if arg1 == "WIM" then
    		if WIM_ChatFrame_MessageEventHandler_orig ~= nil then
    			...
    			--WIM_ChatFrame_MessageEventHandler = ChatFilter
    	...
    	elseif arg1 == "WhisperFilter" then
						...
						if event == "CHAT_MSG_CHANNEL" then
							...
							if arg9 ~= nil then
								...
								for index, value in pairs(this.channelList) do
									...
									for str in string.gmatch(arg9, "[^%s]+") do
				...
				if ChatFrameID ~= nil then
					if event == "CHAT_MSG_SYSTEM" then
						...
						if string.find(arg1,"Level.%d+.* -") ~= nil and hooked then
							...
							local i = 0
							local lvl
					        for str in string.gmatch(arg1, "[^%s]+") do
					        	if i == 2 then
					        		lvl = str
					        	end
					        	i = i + 1
			...
			local function whisperHandler(msg, editbox)
				...
				local parse = {}
				local i = -1
				for cmd in string.gmatch(msg, "[^%s]+") do
					i = i +1
					parse[i] = cmd
				end
